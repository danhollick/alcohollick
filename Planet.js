...
place(objArray) {
  for(var i = 0; i < objArray.length; i++){
        if(this.id !== i+7){ //dont do the check if it is looking at itself
          this.hit = collideCircleCircle(this.x, this.y, this.d, objArray[i].x, objArray[i].y, objArray[i].d)
          this.isCenter = collideCircleCircle(this.x,this.y,this.d,windowWidth/2,windowHeight/2,40) //colliding with anything?
